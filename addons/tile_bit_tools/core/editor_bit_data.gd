...
func apply_template_bit_data(template_bit_data : TemplateBitData, p_terrain_set : int, terrain_mapping : Dictionary) -> G.Errors:
	...
	tile_shape = template_bit_data.tile_shape
	tile_offset_axis = template_bit_data.tile_offset_axis
	terrain_mode = template_bit_data.terrain_mode
	if tile_set.tile_shape != tile_shape || \
		tile_set.tile_offset_axis != tile_offset_axis || \
		tile_set.get_terrain_set_mode(terrain_set) != terrain_mode:
